package com.lucky.rocketmq.validate;
 ...
 * @date 2014-2-25
 */
public class CmdValidator extends MQAdminImpl implements BeanPostProcessor,
		ApplicationListener<ContextRefreshedEvent> {

	public CmdValidator(MQClientInstance mQClientFactory) {
				...
				if (method.isAnnotationPresent(CmdTrace.class)) {
					...
					if (method2cmd.get(methodName) == null) {
						...
						throw new IllegalStateException(methodName + " = {"
								+ method2cmd.get(methodName).getName() + ","
								+ cmdClazz.getName() + "}");
					}
					if (cmd2method.get(cmdClazz) == null) {
						cmd2method.put(cmdClazz, methodName);
					} else {
						throw new IllegalStateException(cmdClazz + " = {"
								+ cmd2method.get(cmdClazz) + "," + methodName
								+ "}");
					}
				}
			}
