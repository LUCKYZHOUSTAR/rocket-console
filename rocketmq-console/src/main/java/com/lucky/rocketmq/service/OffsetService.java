package com.lucky.rocketmq.service;

import static com.lucky.rocketmq.common.Tool.bool;
import static com.lucky.rocketmq.common.Tool.str;
...
import com.lucky.rocketmq.validate.CmdTrace;

/**
 * @see com.lucky.rocketmq.tools.command.offset.ResetOffsetByTimeOldCommand
 * 
...
public class OffsetService extends AbstractService {
	...
	public Table resetOffsetByTime(String consumerGroup, String topic,
			String timeStampStr, String forceStr) throws Throwable {
		Throwable t = null;
		...
		try {
			...
			List<RollbackStats> rollbackStatsList = defaultMQAdminExt
					.resetOffsetByTimestampOld(consumerGroup, topic, timestamp,
							force);
			// System.out
			// .printf(
			...
			// "#rollbackOffset" //
			// );
			String[] thead = new String[] { "#brokerName", "#queueId",
					"#brokerOffset", "#consumerOffset", "#timestampOffset",
					"#rollbackOffset" };
			...
			for (RollbackStats rollbackStats : rollbackStatsList) {
				...
				tr[0] = UtilAll.frontStringAtLeast(
						rollbackStats.getBrokerName(), 32);
